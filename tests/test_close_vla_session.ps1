...
if((Get-Command ssh).CommandType -eq 'Function'){throw 'Do not replace an existing SSH function'}
...
function ssh{
    ...
    if($taskCommand -like '*awq_scope_analysis.py*'){$global:LASTEXITCODE=1;return}
...
$taskInvoke=@{ResultDirectory='/root/autodl-tmp/qvla-repro/eval/awq-scope-shard-test';
    ...
    RawRelativeRoot='results/closure-workflow-test';Execute=$true}
try{
    $taskRejected=$false
    ...
    Write-Output '4 mocked closure failure/receipt checks passed; no real SSH or shutdown'
}finally{
