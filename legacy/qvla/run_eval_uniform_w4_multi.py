...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=(4, 8, 16),
        default=16,
        help="Dynamic fake-quant activation width; 16 disables activation quantization.",
    )
...
def make_input_scale_hook(
    ...
    def hook(_module, args):
        ...
        if activation_bits < 16:
            ...
            activation_scale = (
                scaled_float.abs()
                .amax(dim=reduce_dims, keepdim=True)
                .clamp_min(1e-8)
                / qmax
            )
            scaled = (
                torch.clamp(
                    torch.round(scaled_float / activation_scale),
                    qmin,
                    qmax,
                )
                * activation_scale
    ...
    for name, entry in entries.items():
        ...
        handles.append(
            module.register_forward_pre_hook(
                make_input_scale_hook(module, input_scale, activation_bits)
            )
        )
