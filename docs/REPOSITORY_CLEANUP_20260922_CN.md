...
| 项目 | 处理 | 核验 |
|---|---|---|
| 50 份字节完全相同的统计与源码副本 | 旧分片路径移除，留下 4 份 SHA 命名规范副本 | 原始字节 SHA256 逐项相等；[映射清单](../results/indexes/DEDUPLICATED_FILES_20260922.json)记录全部旧路径、哈希和字节数 |
| 综合报告的重复数值副本 | `结果数据.json` 改为 `EVIDENCE_INDEX.json`，只索引原件并保留权威汇总 | 源文件现按规划模块位于 `results/experiments/` 与 `reports/experiments/`；114 份来源与 1000 条配对 scope 记录仍由生成器核对 |
...
- 综合报告统一为 [`reports/summaries/20260918-experiment-overview/`](../reports/summaries/20260918-experiment-overview/)：`README_CN.md` 是结论，`EXPERIMENT_LOG_CN.md` 是按实验名称的证据索引，`EVIDENCE_INDEX.json` 是机器可读的哈希清单。
...
- `results/shared/source-snapshots/` 保存不可变源码快照；`results/shared/calibration/` 保存共享统计。不同版本源码与统计留在原实验目录。
...
python reports/experiments/p0-foundation-baselines/20260917-107-baseline-continuation/manifest.py --verify
python reports/experiments/p0-foundation-baselines/20260918-107-awq-continuation/manifest.py --verify
