...
当前复现的五个 LIBERO OFT 检查点，应以 `openvla-oft/` 为主。`openvla/` 用于理解原始 VLA 与论文另一组结果；`UniVLA/` 暂时只需了解定位，不要混装其依赖。
...
下面列出复现与理解主线所需的文件。仓库继承了大量训练框架和 OXE 数据配置；逐个机械列出所有辅助文件反而会遮住主线，因此按职责归类。
