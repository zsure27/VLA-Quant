...
> **2026-09-26 P2.5 覆盖：** data80 同时改变轨迹覆盖与总优化步数；当前 50 回合分片的
> 12L 与 14L 均为 43/50，不能用于 W4 恢复率。后续先执行轨迹级 `router_dev`、
> compute-vs-coverage A/B/C/D 和学生实际访问观测上的 BF16 同观测诊断。具体执行与门禁见
> [`P2_5_ON_POLICY_ALIGNMENT_PLAN_20260926_CN.md`](P2_5_ON_POLICY_ALIGNMENT_PLAN_20260926_CN.md)。
> 在 P2.5 解决目标/分布错配前不进入 P4/P5，本文件中与该顺序冲突的旧队列不再执行。
...
| 14L | 8–15、18–23 | 约 430/500（历史 431，待来源审计） | 静态部署膝点候选 |
| 12L | 8–15、20–23 | 约 411/500（历史 412，待来源审计） | 主 PEFT 恢复实验底座 |

禁止自动运行 13 层、11 层、其他 14 层组合或更多粗粒度 W4 岛搜索。14L 不是唯一 Pareto 最优点；它只是当前数据支持的静态部署候选。

12L 相对 14L 只将 blocks 18–19 从 W4 改为 W2；两个完整复跑口径中的差值均约为
19/500，但精确分子须先完成来源审计。核心问题固定为：**能否让 blocks 18–19 保持 W2，
用远小于两个完整 W4 block 的 PEFT 存储接近 14L，并在学生自己访问的状态上保持正确？**
其他量化设置在该实验中不得改变。
...
4. 用轨迹级 `router_dev` 替代历史 32 帧主门禁，并完成 A/B/C/D compute-vs-coverage 控制。
5. 在任何专家训练前完成 P2.5 学生访问观测上的 BF16 同观测诊断；确认 covariate shift 时先做
   同参数量 student-state distillation。
6. 只有 shared 方法有稳定正收益，或 P2.5 排除目标/分布错配后仍出现强且可复现的上下文冲突，
   才提出两个等预算专家。
7. 只有专家互补跨 seed 稳定，才训练使用 `H^(17)` 的最小 Top-1 Router。
