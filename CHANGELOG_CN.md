...
- 新增自适应实验与备份分析协议：按配对闭环、失败模式、不确定性和成本调整下一实验，要求每轮备份同时保存数据、分析、gate结论和恢复证据。
- 将“测试实验”历史中的证据边界固化：rollout不称epoch、开发初态不称盲测、混合精度不称纯W2、离线MSE/SVD/oracle不替代闭环。
- 仓库检查器排除被Git忽略的本地完整归档，避免大备份文件被误判为普通Git资产。
