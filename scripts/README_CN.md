...
| 状态与配对审计 | `awq_scope_status.py`、`baseline_shard_status.py`、`verify_awq_scope_backup_local.py`、`verify_deduplicated_results.py`、`verify_session_manifests.py` |
| 归档与收尾 | `backup_active_diagnostics.py`、`vla_push_local.ps1`、`close_vla_session.ps1`、`vla_shutdown_remote.py` |
...
`close_vla_session.ps1` 默认只预览，必须显式传入 `-Execute` 才会按身份、结果、备份、SHA、GitHub 认证与推送、原生关机回执的顺序执行。脚本不验证平台控制台 OFF 或停止计费；回执与平台状态必须分开表述。当前只依据用户已开启实例的会话进行收尾，不设置定时关机或自动开机。
