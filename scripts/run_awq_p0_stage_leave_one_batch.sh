...
BATCH=$ROOT/eval/awq-p0-stage-leave-one-batch-$(date +%Y%m%d-%H%M%S)-$$
mkdir -p "$BATCH"; printf '%s\n' "$BATCH" > "$ROOT/eval/LATEST_AWQ_P0_STAGE_LEAVE_ONE.txt"
for layers in \
...
printf '{"status":"EXECUTION_COMPLETE","runs":3,"episodes_per_run":10}\n' > "$BATCH/complete.json"
