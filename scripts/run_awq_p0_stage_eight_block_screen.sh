...
BATCH=$ROOT/eval/awq-p0-stage-eight-block-screen-$(date +%Y%m%d-%H%M%S)-$$
mkdir -p "$BATCH"; printf '%s\n' "$BATCH" > "$ROOT/eval/LATEST_AWQ_P0_STAGE_EIGHT.txt"
for layers in 8,9,10,11,20,21,22,23 12,13,14,15,20,21,22,23; do
  bash "$RUNNER" "$layers" 5 1
  cat "$ROOT/eval/LATEST_AWQ_P0_STAGE.txt" >> "$BATCH/RUNS.txt"
done
printf '{"status":"EXECUTION_COMPLETE","runs":2,"episodes_per_run":10}\n' > "$BATCH/complete.json"
