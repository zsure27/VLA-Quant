...
payload={"schema_version":"1.0","question":"Does the existing shared fixed-code Scale-PEFT checkpoint transfer its offline gain to paired closed-loop success?","status":"PREREGISTERED_BEFORE_EXECUTION","suite":"libero_spatial","initial_state_offset":offset,"trials_per_task":count,"task_count":10,"episodes_per_config":count*10,"seed":0,"env_seed":0,"seed_protocol":"paired","configs":{"C0":{"w4_layers":"8-15,20-23","scale":False},"C1":{"w4_layers":"8-15,20-23","scale":True},"C2":{"w4_layers":"8-15,18-23","scale":False}},"scale_state":{"path":str(state),"sha256":hashlib.sha256(state.read_bytes()).hexdigest(),"bytes":state.stat().st_size},"classification":"historical/development closed-loop evidence; not final blind evaluation"}
...
run_config C1-12L-scale '8,9,10,11,12,13,14,15,20,21,22,23' --awq-scale-peft-state "$SCALE_STATE"
run_config C0-12L '8,9,10,11,12,13,14,15,20,21,22,23'
run_config C2-14L '8,9,10,11,12,13,14,15,18,19,20,21,22,23'
printf '{"status":"EXECUTION_COMPLETE","configs":["C0-12L","C1-12L-scale","C2-14L"]}\n' > "$OUT/complete.json"
