...
def main() -> None:
    ...
    for path in root.rglob("*"):
        if path.is_file() and path.resolve() != Path(__file__).resolve() and path.suffix.lower() in {".py", ".sh", ".json", ".md", ".txt", ".csv"}:
    ...
    print(f"仓库检查通过：targets=422, python={len(list(root.rglob('*.py')))}, json={len(json_paths)}, skipped={skipped}")
