...
set -Eeuo pipefail
OFFSET=${1:?official initial state offset}; COUNT=${2:?states per task}
[[ "$OFFSET" =~ ^[0-9]+$ && "$COUNT" =~ ^[0-9]+$ ]] || exit 2
(( COUNT>0 && OFFSET+COUNT<=50 )) || exit 2
MODE=language-w2-g64-no-clip
...
PROFILE=$ROOT/artifacts/awq-language-g64-no-clip-derived/w2-g64-language-no-clip.pt
