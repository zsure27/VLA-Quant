...
set -Eeuo pipefail
MODE=${1:?vision-w4|vision-w2|language-w2-clip|language-w2-no-clip}
OFFSET=${2:?official initial state offset}; COUNT=${3:?states per task}
...
bits=2; candidate=profile; extra=()
case "$MODE" in
 vision-w4) scope=vision; bits=4 ;;
 vision-w2) scope=vision; candidate=w2-no-clip-primary-g64; extra=(--awq-primary-group64-profile "$G64") ;;
 language-w2-clip) scope=language ;;
 language-w2-no-clip) scope=language; candidate=w2-no-clip-primary-g64; extra=(--awq-primary-group64-profile "$G64") ;;
 *) exit 2 ;;
esac
...
sha256sum "$OVERLAY/qvla/run_eval_official_quant.py" "$OVERLAY/oft/experiments/robot/libero/run_libero_eval.py" "$BASE/profiles/w$bits.pt" > "$OUT/CONTRACT_SHA256SUMS.txt"
if [[ "$candidate" != profile ]]; then sha256sum "$G64" >> "$OUT/CONTRACT_SHA256SUMS.txt"; fi
command=(python "$OVERLAY/qvla/run_eval_official_quant.py" --method awq --weight-bits "$bits" --activation-bits 16
 ...
 --profile "$BASE/profiles/w$bits.pt" --official-root "$ROOT/src/official-quantization"
