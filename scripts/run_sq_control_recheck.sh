...
CASES=${CASES:-base}
case "$CASES" in
 base) names=(repeat smooth-only); thresholds=(1e-10 1e-4) ;;
...
test ! -e "$OUT"
mkdir -p "$OUT"
printf '%s\n' "$OUT" > "$ROOT/artifacts/LATEST_SQ_CONTROLS_RECHECK.txt"
...
if [[ "$CASES" == base ]]; then
 run repeat repeat
 run smooth-only smoothquant --profile-dir "$OLD/profiles/sq"
else
