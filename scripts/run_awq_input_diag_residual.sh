...
REPO=/root/VLA-Quant
OVERLAY=$ROOT/overlays/input-diag-recovery
BASE=$ROOT/artifacts/awq-spatial-20260912-163735-1136
VALIDATION=$ROOT/artifacts/awq-validation-20260913-132827-1162
OUT=$ROOT/artifacts/awq-input-diag-residual-$(date +%Y%m%d-%H%M%S)-$$
mkdir -p "$OUT"
printf '%s\n' "$OUT" > "$ROOT/artifacts/LATEST_INPUT_DIAG_RESIDUAL.txt"
...
common=(--checkpoint "$ROOT/models/openvla-7b-oft-finetuned-libero-spatial"
 ...
 --awq-residual-layers 8,9,10,11,12,13,14,15 --awq-residual-rank 8 --awq-residual-calibration-dir "$ROOT/calib/action-space-balanced/libero-512")
for rank in all action; do
 printf '%q ' python "$OVERLAY/diagnostics/probe.py" "${common[@]}" --awq-residual-token-scope "$rank" --output "$OUT/rank-$rank" > "$OUT/rank-$rank.command.txt"
 printf '\n' >> "$OUT/rank-$rank.command.txt"
 python "$OVERLAY/diagnostics/probe.py" "${common[@]}" --awq-residual-token-scope "$rank" --output "$OUT/rank-$rank" 2>&1 | tee "$OUT/rank-$rank.console.log"
done
printf '{"status":"MEASUREMENT_COMPLETE","training_steps":0,"note":"Two 32-frame input-diagonal residual capacity probes; frozen W2, no rollout, no trained PEFT claim."}\n' > "$OUT/complete.json"
