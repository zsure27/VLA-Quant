...
set -Eeuo pipefail
RECIPE=${1:?original|g64-attention-no-clip}; OFFSET=${2:?official initial state offset}; COUNT=${3:?states per task}
...
case "$RECIPE" in
 original) PROFILE=$ROOT/artifacts/awq-spatial-20260912-163735-1136/profiles/w2.pt ;;
 g64-attention-no-clip) PROFILE=$ROOT/artifacts/awq-language-g64-family-no-clip-derived/w2-g64-language-no-clip-attention.pt ;;
 *) exit 2 ;;
esac
bits=2; candidate=profile; scope=all; extra=()
...
if [[ -f "${PROFILE%.pt}.intervention.json" ]]; then cp "${PROFILE%.pt}.intervention.json" "$OUT/profile-intervention.json"; fi
