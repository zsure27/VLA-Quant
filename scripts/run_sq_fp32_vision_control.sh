...
REPO=/root/VLA-Quant
OVERLAY=$ROOT/overlays/fp32-vision-smoothing
OLD=$ROOT/artifacts/controls-20260911-234754-1133
OUT=$ROOT/artifacts/sq-fp32-vision-$(date +%Y%m%d-%H%M%S)-$$
mkdir -p "$OUT"
printf '%s\n' "$OUT" > "$ROOT/artifacts/LATEST_SQ_FP32_VISION.txt"
...
common=(--checkpoint "$ROOT/models/openvla-7b-oft-finetuned-libero-spatial"
 ...
 --smoothing-selection no-language --smoothing-vision-fp32)
run() {
 local name=$1; shift
...
printf '{"status":"MEASUREMENT_COMPLETE","note":"Whole FP32 vision control vs BF16, then smoothing vs same FP32 path; W16A16 only, no low-bit/PEFT baseline claim."}\n' > "$OUT/complete.json"
