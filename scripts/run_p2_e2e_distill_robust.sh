...
SESSION=${VLA_SESSION:-20260926-107-p2-robust-e2e-distill}
...
OUT="$OUT_ROOT/exact12l-response-svd-r8-smoothl1-e2e200-offline32"
test ! -e "$OUT"
mkdir -p "$OUT_ROOT"
...
args=(
  ...
  --awq-e2e-distill-steps 200 --awq-e2e-distill-learning-rate 0.0001
...
{
  "schema_version": "1.0",
  "question": "Does a robust action-chunk objective prevent rare offline outliers from dominating exact-12L rank8 Recovery LoRA training?",
  "status": "PREREGISTERED_BEFORE_EXECUTION",
  "backbone": "awq-w2a16-12l-mixed-spatial-v1",
  "target_blocks": [18, 19],
  "single_major_variable": "normalized action loss: MSE -> Smooth L1(beta=0.1)",
  "fixed_controls": {"rank": 8, "initialization": "Response-SVD", "training_samples": 16, "steps": 200, "learning_rate": 0.0001, "offline_frames": 32},
...
printf '{"status":"COMPLETE","objective":"normalized_action_smooth-l1_8x7","beta":0.1,"training_steps":200}\n' > "$OUT_ROOT/complete.json"
