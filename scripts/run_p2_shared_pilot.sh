...
run_probe exact12l-baseline-offline32

shared=(
  --awq-residual-layers 18,19 --awq-residual-rank 8
  --awq-residual-calibration-dir "$CALIBRATION" --awq-residual-calibration-count 16
  --awq-residual-token-scope action --awq-residual-train-steps 200
  --awq-residual-learning-rate 0.001
)
run_probe exact12l-standard-zero-r8-train200-offline32 "${shared[@]}"
run_probe exact12l-response-svd-r8-train200-offline32 "${shared[@]}" --awq-residual-response-svd
run_probe exact12l-fixed-code-scale-train200-offline32 \
  --awq-scale-peft-layers 18,19 \
  --awq-residual-calibration-dir "$CALIBRATION" --awq-residual-calibration-count 16 \
  --awq-residual-token-scope action --awq-scale-peft-train-steps 200 \
  --awq-scale-peft-learning-rate 0.001

printf '{"status":"COMPLETE","backbone":"awq-w2a16-12l-mixed-spatial-v1","target_blocks":[18,19],"rank":8,"training_steps":200,"calibration_trajectories":16,"note":"Exact language-G64 layer-response pilot; offline agreement is not closed-loop success."}\n' > "$OUT_ROOT/exact12l-layer-complete.json"
