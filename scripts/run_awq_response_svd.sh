...
OUT=$ROOT/artifacts/awq-response-svd-$(date +%Y%m%d-%H%M%S)-$$
mkdir -p "$OUT"
printf '%s\n' "$OUT" > "$ROOT/artifacts/LATEST_RESPONSE_SVD.txt"
...
for rank in 8 16; do
 printf '%q ' python "$OVERLAY/diagnostics/probe.py" "${common[@]}" --awq-residual-token-scope action --awq-residual-response-svd --awq-residual-rank "$rank" --output "$OUT/rank-$rank" > "$OUT/rank-$rank.command.txt"
 printf '\n' >> "$OUT/rank-$rank.command.txt"
 python "$OVERLAY/diagnostics/probe.py" "${common[@]}" --awq-residual-token-scope action --awq-residual-response-svd --awq-residual-rank "$rank" --output "$OUT/rank-$rank" 2>&1 | tee "$OUT/rank-$rank.console.log"
done
printf '{"status":"MEASUREMENT_COMPLETE","training_steps":0,"note":"Two 32-frame action-response SVD initialization probes; frozen W2, no rollout, no trained PEFT claim."}\n' > "$OUT/complete.json"
