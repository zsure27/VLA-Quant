...
def main():
    ...
    with tarfile.open(backup / "code-overlay.tar.gz", "w:gz") as tar:
        for name in ("language-stage-rescue", "closed-loop-stage-rescue", "language-family-rescue", "closed-loop-fixed-coordinates", "low-rank-recovery", "fp32-smoothing-pairs", "input-diag-recovery", "response-svd-recovery", "baseline-shards", "fp32-vision-smoothing", "awq-p0-stage-20260923"):
    ...
    for directory in selected:
        ...
        for console in directory.glob("*/console.log"):
            ...
            for name in paths:
                relative = Path(name)
                if relative.parts and relative.parts[0] == "rollouts":
                    relative = Path(*relative.parts[1:])
                candidates = [(video_root / relative).resolve() for video_root in video_roots]
                path = next((candidate for candidate in candidates if candidate.is_file()), None)
                if path is None or not any(video_root in path.parents for video_root in video_roots):
                    raise SystemExit("Missing or unsafe rollout video: " + str(path))
                video_root = next(root_path for root_path in video_roots if root_path in path.parents)
                videos.append({"path": str(path), "bytes": path.stat().st_size,
                               "sha256": sha(path), "source_console": str(console),
                               "archive_name": "rollouts/" + path.relative_to(video_root).as_posix()})
    ...
    with tarfile.open(backup / "session-results.tar.gz", "w:gz") as tar:
        ...
        for video in videos:
            ...
            tar.add(path, arcname=video["archive_name"])
    large = []
