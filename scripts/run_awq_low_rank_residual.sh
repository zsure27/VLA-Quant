...
REPO=/root/VLA-Quant
OVERLAY=$ROOT/overlays/low-rank-recovery
BASE=$ROOT/artifacts/awq-spatial-20260912-163735-1136
VALIDATION=$ROOT/artifacts/awq-validation-20260913-132827-1162
OUT=$ROOT/artifacts/awq-low-rank-residual-$(date +%Y%m%d-%H%M%S)-$$
mkdir -p "$OUT"
printf '%s\n' "$OUT" > "$ROOT/artifacts/LATEST_LOW_RANK_RESIDUAL.txt"
...
common=(--checkpoint "$ROOT/models/openvla-7b-oft-finetuned-libero-spatial"
 ...
 --awq-residual-layers 8,9,10,11,12,13,14,15)
for rank in 4 8; do
 printf '%q ' python "$OVERLAY/diagnostics/probe.py" "${common[@]}" --awq-residual-rank "$rank" --output "$OUT/rank-$rank" > "$OUT/rank-$rank.command.txt"
 printf '\n' >> "$OUT/rank-$rank.command.txt"
 python "$OVERLAY/diagnostics/probe.py" "${common[@]}" --awq-residual-rank "$rank" --output "$OUT/rank-$rank" 2>&1 | tee "$OUT/rank-$rank.console.log"
done
printf '{"status":"MEASUREMENT_COMPLETE","training_steps":0,"note":"Two 32-frame low-rank SVD residual capacity probes; frozen W2, no rollout, no trained PEFT claim."}\n' > "$OUT/complete.json"
