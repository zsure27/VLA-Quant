...
OUT="$OUT_ROOT/exact12l-fixed-code-scale-train200-state-offline32"
CALIBRATION="$ROOT/backups/experiments/p1-data-contract/20260925-107-p1-data-contract/peft-train-calibration16"
test ! -e "$OUT"
mkdir -p "$OUT_ROOT"
...
/root/miniconda3/envs/qvla-oft/bin/python -u "$REPO/diagnostics/probe.py" \
  ...
  --awq-scale-peft-layers 18,19 --awq-residual-calibration-dir "$CALIBRATION" \
  --awq-residual-calibration-count 16 --awq-residual-token-scope action \
  --awq-scale-peft-train-steps 200 --awq-scale-peft-learning-rate 0.001 \
  --output "$OUT" 2>&1 | tee "$OUT_ROOT/exact12l-scale-state.console.log"
