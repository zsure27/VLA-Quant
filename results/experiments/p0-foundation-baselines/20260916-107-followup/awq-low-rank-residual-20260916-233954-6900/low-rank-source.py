...
def attach_residual(linear, teacher_weight, rank, seed):
    ...
    with torch.no_grad(),torch.random.fork_rng(devices=devices):
        ...
        u,s,v=torch.svd_lowrank(residual,q=min(rank+8,min(residual.shape)),niter=2)
        a=v[:,:rank].T
        b=u[:,:rank]*s[:rank]
    ...
    return {"rank":rank,"adapter_parameters":sum(p.numel() for p in branch.parameters()),
            ...
            "init":"randomized_svd_weight_residual_not_training","seed":seed}
