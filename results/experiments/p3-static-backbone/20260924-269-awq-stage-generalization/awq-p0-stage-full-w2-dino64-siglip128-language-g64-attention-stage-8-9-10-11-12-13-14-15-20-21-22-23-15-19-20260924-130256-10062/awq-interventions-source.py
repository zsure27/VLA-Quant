...
def plan(entries, meta, clip_scope, w4_layers, w4=None, allow_group_mismatch=False):
    ...
    if w4:
        ...
        keys = ["checkpoint_identity", "sample_names", "sample_sha256", "group_size",
                    "seed", "model_source_sha256", "llm_attention", "official_sources",
                    "implementation_sources"]
        if allow_group_mismatch:
            keys.remove("group_size")
        for key in keys:
            if meta[key] != m4[key]:
...
def attention_visual_stage_plan(entries, meta, group64_peer, w4_peer, w4_layers):
    """DINO G64/SigLIP G128 plus G64 attention-only language with whole W4 blocks."""
    if not w4_layers:
        raise ValueError("至少选择一个完整 W4 block")
    selected, other = group64_peer
    comparison = dict(other, group_size=meta["group_size"])
    check_peer(entries, meta, selected, comparison)
    scales, targets, removed = plan(
        selected, other, "attention", w4_layers, w4_peer, allow_group_mismatch=True
    )
    visual = vision_plan(entries, meta, 2)
    visual.update(primary_group_plan(entries, meta, group64_peer))
    targets.update(visual)
    if len(targets) != 422:
        raise ValueError("stage 候选必须覆盖422目标")
    return scales, targets, removed
...
def family_precision_plan(entries, meta, layers, family):
    ...
    """
    if not layers or family not in ("attention", "mlp", "all"):
    ...
    for name, (entry, bits, group) in targets.items():
        ...
        if layer in layers and (family == "all" or marker in name):
            targets[name] = (entry, 4, group)
            selected.append(name)
    if len(selected) != len(layers)*{"attention": 4, "mlp": 3, "all": 7}[family]:
