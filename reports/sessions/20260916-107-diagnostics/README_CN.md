...
日期：2026-09-16。最新实验实例为用户从014克隆的**107机，SSH端口31263**；专用公钥认证成功，未使用或保存用户密码。容器hostname为`autodl-container-8b78499521-41be183e`，这是连入容器标识，不以它替代未经控制台核对的完整实例ID。本节记录四段离线、两组闭环和四项SQ控制的实际结果；模块族细分与收尾将在完成后补充。
...
额度查询连续失败，无法确认剩余和下一长测试的安全预算，因此不再派发，主动收尾。GitHub同步及平台关机状态待最后操作核验后补录；AutoDL浏览器会话已过期，已请求用户自行登录以供平台状态核验，不使用此问题阻止SSH安全收尾。未删除唯一结果、模型或原校准集。
