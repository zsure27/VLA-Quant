...
日期：2026-09-16。最新实验实例为用户从014克隆的**107机，SSH端口31263**；专用公钥认证成功，未使用或保存用户密码。容器hostname为`autodl-container-8b78499521-41be183e`，这是连入容器标识，不以它替代未经控制台核对的完整实例ID。本轮四段离线、两组闭环、四项SQ控制及两项模块族细分，共12项有限测量均已完成。
...
北京时间19:44:20，107关机工具核验归档SHA256、当前base bundle及GPU无计算进程后，执行已验证的AutoDL原生supervisor停止机制；服务器随后主动断开SSH。具体请求回执见[closure.json](closure.json)。**关机请求已执行，平台OFF/停止计费尚未独立核验**：浏览器仍在登录页，已请用户自行登录供核验，未代用户执行登录。不得将SSH断开写成平台已确认关机。没有销毁107、清空Trash、删除模型/原校准集或唯一结果。
