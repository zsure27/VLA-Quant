...
result=dict(variants=summary,covered_initial_indices=sorted(set(r["init_state_index"] for r in pairs)),paired_episodes=len(pairs),bf16_only=only_b,w4_only=only_w,mcnemar_exact_p=min(1.,2*sum(math.comb(n,k) for k in range(min(only_b,only_w)+1))/2**n) if n else 1.,note="Fixed tasks, paired official init indices; p value not equivalence; fake quant precision only")
