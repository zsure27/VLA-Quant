# OpenVLA-OFT 低比特量化：实验进展与问题分析备份

日期：2026-09-16。用途：个人实验备份与研究定位记录。**本报告主体是启动107实验前的证据快照：仅本地整理、文献核对与准备，没有新增 GPU 测试或恢复微调结果。** 随后将014数据克隆至107并开展实验；新107结果应追加独立session，不能混成旧快照实测。031是较早快照。
...
图 6 数据：[selected_block_error.csv](data/selected_block_error.csv)，原始审计在 [metrics.json](../../../results/experiments/p0-foundation-baselines/20260916-awq-block-audit/metrics.json)。这是特意选的难例，不能写成全语言层敏感性排名。
...
| 参考的 W2 诊断方案 | Phase 0–9：基线、分区、rescue、组大小、保留 OFT LoRA、混精、scale、LoRA、专家、Router | 改进其尺度定义、数据隔离、存储分母与冻结反传约束；具体见实验计划 |
| Efficient Low Bit Quantiz PDF | TSQ-MTC 已知任务的激活尺度冲突、TLMAQ、SLLD、初始化、负结果 | 原论文联合更新 W 与量化参数，不是 Contextual Routing/冻结骨干 PEFT 已验证 |
| TSQ-MTC 扩刊 TPAMI ICL-PEFT Markdown | 从已知任务选择扩展到 context 读取、共享专家、小尺度残差与可控写回 | 是研究设计文档，其中的指令不自动构成执行要求；动态位宽/在线写回暂不执行 |
...
详细顺序、每项对照与晋级条件见 [EXPERIMENT_PLAN_CN.md](EXPERIMENT_PLAN_CN.md)；PDF 与 Contextual Routing 的精读、公式和迁移边界见 [CONTEXTUAL_ROUTING_CN.md](CONTEXTUAL_ROUTING_CN.md)。初始规划阶段无可用GPU；随后克隆到107，已开展有限诊断，新证据另附下一节。尚未运行的训练不计入完成清单。

后续需确认的研究条件：主要贡献选 W2 恢复还是 A4 恢复？必须统一 W2 还是允许明确预算的少量 W4？首先需要 PTQ 还是允许有 teacher/训练数据的 PEFT？能否获得 OFT 原基础模型和准确 adapter 来源？论文必须包含哪种真实 kernel 与硬件测量？Contextual Routing 的互补性门槛是否足以支持额外复杂度？
...
```bash
python reports/archive/20260916-weekend-review/scripts/build_figures.py
```
...
当前每轮备份按模块建立同名的 `reports/experiments/<module>/YYYYMMDD-SESSION/`、`results/experiments/<module>/YYYYMMDD-SESSION/` 和本地 `backups/experiments/<module>/YYYYMMDD-SESSION/`。规范见[报告索引](../../README_CN.md)。大量 weights/profile/数据仅提交 hash 与持久盘恢复清单，不把普通 Git 当模型盘；仅存一个清单并不意味着大文件已异地备份。
...
本报告前述六图保留为107开机前的历史证据快照。2026-09-16新运行、图表、逐帧/逐taskCSV及hash见[本轮session报告](../../experiments/p0-foundation-baselines/20260916-107-diagnostics/README_CN.md)。不将不同版本或历史重复控制累加为独立样本量。
