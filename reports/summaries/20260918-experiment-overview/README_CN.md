...
> **2026-09-25 主线更新：** 后续执行以[PEFT/Contextual Routing 主线协议](../../../docs/PEFT_CONTEXTUAL_ROUTING_EXECUTION_20260925_CN.md)为准。静态 W4 block 搜索已冻结；最新 Spatial500 为 16L 434/500、14L 431/500、12L 412/500。16L 是较强静态参照，14L 是部署膝点候选，12L 是 blocks18–19 shared PEFT 的主底座。本文后部旧“后续安排”中与该协议冲突的内容不再执行。

原汇总更新：2026-09-18；主线覆盖更新：2026-09-25。用于个人实验进展备份与后续研究规划。

本目录保留汇报总结、按实验名称组织的日志、证据索引及摘要图。旧版完整报告、CSV 和制图脚本保存在[历史归档](../../archive/20260916-weekend-review/)；详细报告与原始小结果按 P0–P5 模块分别保存在 [`reports/experiments/`](../../experiments/README_CN.md) 和 [`results/experiments/`](../../../results/experiments/README_CN.md)。
...
![当前基线与完整W2结果](assets/01_baselines.png)

图 1：左侧三个配置均覆盖官方初态 0–49 的固定 Spatial500；右侧均为初态 5–9 的开发 50 回合。两侧样本不同，不能横向当成同一批次比较。源数据来自[正式500汇总](../../experiments/p0-foundation-baselines/20260917-107-baseline-continuation/data/summary.json)和[最新 scope 汇总](../../experiments/p0-foundation-baselines/20260918-107-awq-continuation/data/scope_summary.json)，同时完整收录于本目录结果数据。
...
![W4分任务结果](../../experiments/p0-foundation-baselines/20260917-107-baseline-continuation/figures/01_per_task.png)

图 2：[逐任务和逐回合源数据](../../experiments/p0-foundation-baselines/20260917-107-baseline-continuation/data/)。总体接近不等于逐案例一致，19 个成功结果不同的案例应与视频一起复核。旧开发 50 的 BF16/W4 同为 47/50，以及另一十初态 W4 为 8/10，属于历史开发证据，不与正式 500 累加。
...
![语言离线剪裁诊断](../../archive/20260916-weekend-review/figures/03_language_offline.png)

图 3：[原数据与证据标记](../../archive/20260916-weekend-review/data/language_offline.csv)。诊断有实际逐帧记录；无法复取的既有验证摘要标 `archival_summary`，不补造逐帧值。这些是相对 BF16 教师的归一化动作误差，**不是相对 GT action 的误差**。32×8=256 是相关 chunk 动作步，不是 256 条独立轨迹。
...
![G64剪裁四格实验](../../experiments/p0-foundation-baselines/20260918-107-awq-continuation/figures/07_language_clip_factorial.png)

图 4：[四格 JSON](../../experiments/p0-foundation-baselines/20260918-107-awq-continuation/data/language_clip_factorial.json)。G64 四格保留同一原始 profile 的 scale、校准与其他字段，仅撤销指定 clip；CPU 逐字段/tensor 相等核验通过。G128 与 G64 来自不同搜索 profile，因此其对比同时包含组大小及搜索参数变化，不能解释成单独的组大小因果效应。

注意力方案随后扩展到官方初态 10–19：49/100，BF16 96/100。开发 29/50 与扩展 49/100分开记录。这里“新初态”仅是该配置此前未测过的官方起点；已存在 BF16/W4 参考，不是全项目独立盲测。

![注意力策略扩展验证](../../experiments/p0-foundation-baselines/20260918-107-awq-continuation/figures/08_attention_state_extension.png)

图 5：[分批源数据](../../experiments/p0-foundation-baselines/20260918-107-awq-continuation/data/language_attention_splits.json)。这说明改进在扩展起点仍能部分完成任务，但差距达 47 个百分点。不能把“成功了若干回合”称为 W2 基线已经可用。
...
![视觉W2任务敏感性](../../experiments/p0-foundation-baselines/20260918-107-awq-continuation/figures/05_vision_per_task.png)

图 6：[逐任务 CSV](../../experiments/p0-foundation-baselines/20260918-107-awq-continuation/data/scope_per_task.csv)。task1 为 33/50 对 BF16 50/50；task5 为 40/50 对 47/50；task7 为 40/50 对 45/50。task0/2/3/6/8 为 50/50。下降具有任务集中性，不是所有视觉案例失效，也绝非视觉 W2 无损。视觉约占既定连接目标权重 9.6%，仅量化视觉的成功率不能证明主要语言权重压缩可行。

![task1真实失败轨迹抽帧](../../experiments/p0-foundation-baselines/20260918-107-awq-continuation/figures/vision-w2-task1-contact-350-20260918/task-1-init-07.png)
...
![中段回退的离线效果](../../experiments/p0-foundation-baselines/20260916-107-diagnostics/figures/01_stage_rescue.png)

![中段8–23保护的开发闭环](../../experiments/p0-foundation-baselines/20260916-107-followup/figures/05_stage_08_23_closed_loop50.png)
...
![SQ视觉分支数值控制](../../experiments/p0-foundation-baselines/20260916-107-followup/figures/02_sq_vision_branches.png)

![SQ局部等价性检查](../../experiments/p0-foundation-baselines/20260916-107-followup/figures/03_sq_local_equivalence.png)
...
![响应SVD及同预算初始化](../../experiments/p2-shared-peft/20260917-107-response-svd/figures/01_action_mse.png)

图 12：[响应逐帧、模块几何和汇总数据](../../experiments/p2-shared-peft/20260917-107-response-svd/data/)。权重SVD的rank8只解释很少权重残差；输入RMS加权略改善动作。响应SVD拟合 `ΔW·X_actionᵀ` 的输出主方向，用 `B=U_r、A=U_rᵀΔW` 初始化。它保留输入通道相关性，区别于对角RMS。
...
![专家互补性的诊断上界](assets/02_router_upper_bound.png)
...
[EVIDENCE_INDEX.json](EVIDENCE_INDEX.json)索引114份来源，保留权威汇总、每份源文件的路径、字节数与 SHA256，以及图表和文档哈希。CSV 单元格、逐帧指标和大型逐层统计保留在各自的原始文件中，避免在综合 JSON 中重复存一份；由索引可定位并核验。多个 session 摘要重叠是快照，不增加独立样本量；未测项显式列出。重建命令：`python scripts/build_experiment_summary.py`，只读本地源数据、无需 GPU 或网络。
