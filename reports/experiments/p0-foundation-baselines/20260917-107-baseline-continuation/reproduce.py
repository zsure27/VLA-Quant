...
HERE=Path(__file__).resolve().parent;REPO=HERE.parents[2];RAW=REPO/"results/experiments/p0-foundation-baselines/20260917-107-baseline-continuation";REFERENCE=REPO/"results/experiments/p0-foundation-baselines/20260917-107-baseline-validation"
files=sorted(REFERENCE.glob("awq-baseline-shard-*/paired-results.json"))+sorted(RAW.glob("awq-baseline-shard-*/paired-results.json"))
if not files:raise SystemExit("No completed paired shards; no fabricated figure")
pairs=[];contracts=set()
for p in files:
 d=json.loads(p.read_text(encoding="utf-8"));pairs.extend(dict(row,source_shard=p.parent.name,source_session=p.parent.parent.name) for row in d["paired_episodes"])
...
fig,ax=plt.subplots(figsize=(9,4));x=np.arange(10);ax.bar(x-.18,[r["bf16_successes"]/r["episodes"] for r in rows],.36,label="BF16");ax.bar(x+.18,[r["w4_successes"]/r["episodes"] for r in rows],.36,label="AWQ W4A16");ax.set_xticks(x);ax.set_xlabel("Spatial task ID");ax.set_ylabel("Success rate");ax.set_ylim(0,1.05);ax.legend(loc="lower center",bbox_to_anchor=(.5,1.01),ncol=2);ax.set_title(str(len(pairs))+" matched episodes per variant; fixed profile and seeds",pad=40);fig.tight_layout()
