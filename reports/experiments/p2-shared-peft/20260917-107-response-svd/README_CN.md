...
本轮107重新开机后继续响应SVD预实验。目标是区分“重建权重矩阵”与“补偿动作输入引起的量化响应”两个低秩初始化目标；保持语言AWQ W2G128无额外clip、视觉及action head等保护模块BF16，在语言8–15层56个Linear各添加高精度低秩残差。
...
原始小文件位于[results/experiments/p2-shared-peft/20260917-107-response-svd](../../../../results/experiments/p2-shared-peft/20260917-107-response-svd/)，与本报告相互引用。大型原始输出和源码overlay另外双份备份，不上传模型、校准原件或凭据到普通Git；初始低秩权重本轮未单独保存，恢复需从记录的源码、profile、校准及seed重初始化并核验指标。没有训练checkpoint或优化器状态。
...
北京时间2026-09-17 00:50:14已执行本轮107原生关机请求（supervisor PID 838），SSH随后被远端关闭。关机前GitHub提交8933d11核验通过。**平台OFF及停止计费未独立核验**，待控制台核验；未销毁实例、未删除唯一数据、未设置定时关机。
